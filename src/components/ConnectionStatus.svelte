...
<div class="flex gap-6 items-center" class:hidden={!$isLoggedIn}>
  ...
  <button on:click={switchNetwork} class="flex justify-center items-center select-none bg-red-500 border-2 text-xl font-bold p-2 m-2 rounded-full shadow h-20 w-20 focus:outline-none focus:shadow-outline" style="width: 44px; height: 44px; background: #6EE9D1; box-shadow: 0px 4px 4px rgba(0, 0, 0, 0.25); border-radius: 5px"><img class="" src={networkSwitchIcon} alt="Switch Network" /></button>
  <!-- {/if} -->
</div>
