...
describe('createMailto', () => {
    type TestCase = {
        to: string, subject: string|undefined,body: string|undefined,
        expectedOutput: string,
        expectedErr: boolean,
    }
    const testCases: Array<TestCase> = [
        {to: "foo@bar", subject: "hello", body: 'testing one two three',
            expectedOutput: "to is not an email address: foo@bar", expectedErr: true},
        {to: "foo@bar.com", subject: "hello", body: 'testing one two three',
            expectedOutput: "mailto:foo@bar.com?subject=hello&body=testing%20one%20two%20three", expectedErr: false},
        {to: "nowhere@mozilla.org", subject: "This is the subject", body: 'This is the body',
    ...
    testCases.forEach((tc) => {
        it(`${tc.body}: result = ${tc.expectedOutput}`, () => {
            if (tc.expectedErr) {
                expect(() => createMailto(tc.to, tc.subject, tc.body))
                    .toThrow(tc.expectedOutput)
            } else {
