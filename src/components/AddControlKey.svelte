...
<div id="add-control-key" class:hidden={!(connected && showSelf)} class="action-card basis-1/2">
  ...
  <form class="mt-8">
    ...
    <div class="flex w-350 justify-between">
      <button on:click|preventDefault={addControlKey} class="btn-primary action-btn-l select-none">Add It</button>
      <button on:click|preventDefault={cancelAction} class="btn-cancel action-btn-r select-none">Cancel Add </button>
    </div>
  </form>
</div>
